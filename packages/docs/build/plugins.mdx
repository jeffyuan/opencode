...
declaration or options with `{ namespace, codemode, pinned }`:

- `namespace` prefixes and groups the exposed tool name (and becomes the
  CodeMode path segment).
- `codemode` defaults to `true` and makes the tool available through the
  ...
  provider.
- `pinned` keeps a full signature in compact CodeMode catalogs. Valid only when
  `codemode` is not `false`.
