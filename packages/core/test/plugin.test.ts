...
describe("PluginV2", () => {
  ...
  it.effect("accepts flat Effect draft declarations with namespace and pinned", () =>
    Effect.gen(function* () {
      const plugins = yield* PluginV2.Service
      const registry = yield* ToolRegistry.Service
      const plugin = EffectPlugin.define({
  ...
  it.effect("rejects pinned when codemode is false", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toContain("pinned is only valid when codemode is not false")
    }),
  )
